...
namespace Webcraft\Helpers\Websend;
...
class Websend
{
	...
	public function __construct($config)
	{
    	...
    	$this->password = $config['websend']['password'];
	}
	...
	*/
	...
	{
		if ( $this->connect() ) {
			return $this->doCommandAsConsole($message);
			$this->disconnect();
		}
	...
	public function message($message, $to = 'console')
	{
		if ( $to === 'console' ) {
			$this->console('say ' . $message);
		} else {
			$this->console("m {$to} {$message}");
		}
	}
}

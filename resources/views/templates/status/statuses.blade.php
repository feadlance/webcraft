<div data-id="{{ $status->id }}" data-type="status" class="item ui">
	<div class="header">
		...
		<div class="avatar">
			<img src="https://minotar.net/avatar/{{ $status->user()->username }}/50" alt="User Avatar">
		</div>
		<div class="title">
	...
	<div data-type="comment" class="comments clear-after">
		@if ( Auth::user()->isFriendsWith($user) || Auth::id() === $user->id )
			<form class="post-comment p-relative">
				<div class="user">
					<img src="https://minotar.net/avatar/{{ \Auth::user()->username }}/40" alt="User Avatar">
				</div>
		...
		@foreach ( $status->comments()->orderBy('id', 'asc')->get() as $comment )
			<div data-id="{{ $comment->id }}" class="comment">
				<img src="https://minotar.net/avatar/{{ $comment->user()->username }}/40" alt="User Avatar">
				...
				<div class="bottom">
					...
					@if ( Auth::user()->isFriendsWith($user) || Auth::id() === $user->id )
						<form data-type="reply" class="post-comment-reply p-relative">
							<div class="user">
								<img src="https://minotar.net/avatar/{{ \Auth::user()->username }}/25" alt="User Avatar">
							</div>
				...
				<div class="child-comments">
					@foreach ( $status->comments($comment->id)->orderBy('id', 'asc')->get() as $reply_comment )
						<div data-type="comment" data-id="{{ $reply_comment->id }}" class="comment">
							<img src="https://minotar.net/avatar/{{ $reply_comment->user()->username }}/25" alt="User Avatar">
							<div class="comment-body">
